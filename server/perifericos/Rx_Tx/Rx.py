...
class rx():
    ...
    def __init__(self,mod_num):
        self.rx_gate_pair=rx.n_modules%4
        self.slot=int(mod_num)
        rx.n_modules=rx.n_modules+1               
    ...
    def instances_to_webdict(self):
        vec_webdict= [{'name':'type','type':'Rx'},{'name':'slot','slot':self.slot},{'name':'gate_pair','gate_pair':self.rx_gate_pair},{'name':'gain','gain':self.gain_LO},{'name':'gain_type','gain_type':'bool'}]
        return vec_webdict
    ...
    def select_rx_gate_pair(self):
    
        if self.rx_gate_pair == 1:
            self.clear_pins_latches(GPIOB,GPIOB6)    
            self.clear_pins_latches(GPIOB,GPIOB5)
            
        elif self.rx_gate_pair == 2:
            self.set_pins_latches(GPIOB,GPIOB6)
            self.clear_pins_latches(GPIOB,GPIOB5)     
        
        elif self.rx_gate_pair == 3:
            self.set_pins_latches(GPIOB,GPIOB5)
            self.clear_pins_latches(GPIOB,GPIOB6)
                
        elif self.rx_gate_pair == 4:
            self.set_pins_latches(GPIOB,GPIOB6)
            self.set_pins_latches(GPIOB,GPIOB5)
    
    def select_gain(self):
        
        if self.gain_LO == True:
            self.set_pins_latches(GPIOB,GPIOB0)
            self.clear_pins_latches(GPIOB,GPIOB1) 

        if self.gain_LO == False:
            self.set_pins_latches(GPIOB,GPIOB1)
            self.clear_pins_latches(GPIOB,GPIOB0)                        
